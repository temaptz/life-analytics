...
function* selectDefaultGraph(action) {
    try {
        ...
        if ( selectedGraphId !== lastGraphId ) {
            ...
            yield put({type: actionTypes.SELECT_GRAPH_REQUEST, payload: null});

            const graph = yield call(graphApi.getGraph, selectedGraphId);

            yield put({type: actionTypes.SELECT_GRAPH_SUCCESS, payload: graph});

        }

    } catch (e) {

        yield put({type: actionTypes.SELECT_GRAPH_ERROR, payload: null});
...
function* getGraphPoints(action) {
    try {
        ...
        yield put({type: actionTypes.GET_POINTS_REQUEST, payload: null});

        const points = yield call(pointsApi.getGraphPoints, state.Graph.id);

        yield put({type: actionTypes.GET_POINTS_SUCCESS, payload: points});

    } catch (e) {

        yield put({type: actionTypes.GET_POINTS_ERROR, payload: null});
...
function* updateGraphList(action) {
    try {
        ...
        yield put({type: actionTypes.GET_GRAPH_LIST_REQUEST, payload: null});

        const points = yield call(graphApi.getGraphList);

        yield put({type: actionTypes.GET_GRAPH_LIST_SUCCESS, payload: points});

    } catch (e) {

        yield put({type: actionTypes.GET_GRAPH_LIST_ERROR, payload: null});
